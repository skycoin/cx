...
func (prgrm *CXProgram) adaptPreEvolution (solutionName string) {
	if mod, err := prgrm.GetModule("main"); err == nil {
		if _, err := prgrm.GetFunction("main", "main"); err == nil {
			idx := -1
		...
		if solMod, err := prgrm.GetCurrentModule(); err == nil {
			if mod, err := prgrm.GetModule("main"); err == nil {
				mod.AddFunction(MakeFunction("main"))
				if fn, err := prgrm.GetCurrentFunction(); err == nil {
					fn.AddOutput(MakeParameter("out", "f64"))
					if sol, err := prgrm.GetFunction(solutionName, solMod.Name); err == nil {
						fn.AddExpression(MakeExpression(sol))

						if expr, err := prgrm.GetCurrentExpression(); err == nil {
...
func (prgrm *CXProgram) adaptInput (testValue float64) {
	if fn, err := prgrm.GetFunction("main", "main"); err == nil {
...
func (prgrm *CXProgram) Evolve (solutionName string, fnBag string, inputs, outputs []float64, numberExprs, iterations int, epsilon float64, expr *CXExpression, call *CXCall) error {
	...
	if fn, err := prgrm.GetCurrentFunction(); err == nil {
		...
		for i := 0; i < numberExprs - preExistingExpressions; i++ {
			...
			if i != numberExprs - preExistingExpressions - 1 {
				...
				coreModule, _ := prgrm.GetModule("core")
	...
	if mod, err := prgrm.GetCurrentModule(); err == nil {
		if fn, err := prgrm.GetFunction(solutionName, mod.Name); err == nil {
