...
func opSliceAppend(prgrm *ast.CXProgram, inputs []ast.CXValue, outputs []ast.CXValue) {
	...
	outputSliceOffset := ast.SliceAppendResize(prgrm, inputs[0].FramePointer, out0, inp0, ast.GetDerefSizeSlice(eltInp0), sliceInputsLen)
...
func opSliceRemoveElement(prgrm *ast.CXProgram, inputs []ast.CXValue, outputs []ast.CXValue) {
	...
	outputSliceOffset := ast.SliceRemove(prgrm, fp, out0, inp0, types.Cast_i32_to_ptr(inputs[1].Get_i32(prgrm)), inp0.GetAssignmentElement().Size)
