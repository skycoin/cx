...
const POINTER_SIZE = UI64_SIZE
const InvalidPointer = Pointer(MAX_UINT64)

func Cast_ptr_to_int(value Pointer) int {
	panicIf(uint64(value) > uint64(MAX_INT), CX_RUNTIME_INVALID_CAST)
	return int(value)
}

func Cast_ptr_to_i32(value Pointer) int32 {
	panicIf(uint64(value) > uint64(MAX_INT32), CX_RUNTIME_INVALID_CAST)
	return int32(value)
}

func Cast_ptr_to_ui32(value Pointer) uint32 {
	panicIf(uint64(value) > uint64(MAX_UINT32), CX_RUNTIME_INVALID_CAST)
	return uint32(value)
}
