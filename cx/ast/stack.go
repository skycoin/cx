	...
	for c := cxprogram.CallCounter; c != types.InvalidPointer; c-- {
		...
		for _, expr := range op.Expressions {
			...
			for _, outputIdx := range cxAtomicOp.GetOutputs(cxprogram) {
				...
				} else if output.Type == TYPE_ATOMIC || output.Type == TYPE_POINTER_ATOMIC || output.Type == TYPE_ARRAY_ATOMIC || output.Type == TYPE_POINTER_ARRAY_ATOMIC || output.Type == TYPE_SLICE_ATOMIC {
