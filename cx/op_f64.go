...
func opF64Cast(expr *CXExpression, fp int) {
	inpV0 := ReadF64(fp, expr.Inputs[0])
	out0 := expr.Outputs[0]
	outOffset0 := GetFinalOffset(fp, out0)

	switch out0.Type {
	case TYPE_STR:
		WriteObject(outOffset0, FromStr(strconv.FormatFloat(float64(inpV0), 'f', -1, 64)))
	case TYPE_I32:
	...
	case TYPE_F64:
		WriteMemory(outOffset0, FromF64(inpV0))
	default:
