...
func opUI64ToI8(prgrm *CXProgram) {
	...
	outB0 := FromI8(int8(ReadUI64(fp, expr.Outputs[0])))
...
func opUI64ToI16(prgrm *CXProgram) {
	...
	outB0 := FromI16(int16(ReadUI64(fp, expr.Outputs[0])))
	WriteMemory(GetFinalOffset(fp, expr.Outputs[0]), outB0)
}
...
func opUI64Max(prgrm *CXProgram) {
	...
	inp0 := ReadUI64(fp, expr.Inputs[0])
	inp1 := ReadUI64(fp, expr.Inputs[1])
	if inp1 > inp0 {
		inp0 = inp1
	}
	WriteMemory(GetFinalOffset(fp, expr.Outputs[0]), FromUI64(inp0))
}
