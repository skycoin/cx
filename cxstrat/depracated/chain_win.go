...
func changeName(prgm *CXProgram) {
	/* has magic number: SHA256(cxtweet_changename_magic) on blockchain */
	/* name defaults to wallet address */
	/* name must be [a-zA-Z_][a-zA-Z0-9_]{0, 31} */
	/* it's that simple */
	/* first we need to get the input text */
	expr := prgm.GetExpr()
	fp := prgm.GetFramePointer()

	magic_ := []byte("<|::[]CXTWEET_NAMENAM[]::|>")

	str_ := ReadStr(fp, expr.Inputs[0])
	byts_ := []byte(str_)

	regexpr_ := regexp.MustCompile("[A-Za-z_][0-9A-Za-z_]*")
	if !regexpr_.Match(byts_) {
		fmt.Println("it wasn't good")
		WriteBool(GetFinalOffset(fp, expr.Outputs[0]), false)
		return
	}

	postMessageBlockchain(prgm, append(magic_, byts_...))
}

/**
 * There is now only four ops for adding data to the blockchain.
 * newtweet - adding a raw tweet to the blockchain
 * chainraw - adding a byte string 'as-is' to the blockchain.
 */

func follow(prgm *CXProgram) {
	expr := prgm.GetExpr()
	fp := prgm.GetFramePointer()

	magic_ := []byte("<|::[]CXTWEET_FOLLOWF[]::|>")
...
func getBlock(prgm *CXProgram) {
	...
	//fmt.Println(txid[:])
	wal := wal_.Bytes()
	tweet := []byte(respBody.Body.Transactions[0].Tweet)
	if len(tweet) < 512 {
		tweet = append(tweet, make([]byte, 512-len(tweet))...)
	}
	val := append(append(txid[:], wal...), tweet...)
...
func newRetweet(prgm *CXProgram) {
	expr := prgm.GetExpr()
	fp := prgm.GetFramePointer()

	magic_ := []byte("<|::[]CXTWEET_RETWEET[]::|>")

	var hsh cipher.SHA256
	byts_ := ReadMemory(GetFinalOffset(fp, expr.Inputs[0]), expr.Inputs[0])
	copy(hsh[:], byts_)

	byts := []byte(hsh.Hex())

	var content_ []byte
	content_ = append(magic_, byts...)

	postMessageBlockchain(prgm, content_)
}

func newLike(prgm *CXProgram) {
	expr := prgm.GetExpr()
	fp := prgm.GetFramePointer()

	magic_ := []byte("<|::[]CXTWEET_LIKELIK[]::|>")
...
//3.
func launchCXTweet(prgm *CXProgram) {
