...
const (
	...
	ERRORCODE_TWEETTOOLONG = 1
	ERRORCODE_OK           = 0
	ERRORCODE_TWEETINVALID = 2
	ERRORCODE_BADOBJECT    = 3

	DBG_CHECK_CACHE_ERR = false
)
...
ID: <a href="http://127.0.0.1:6969/tweet?id=%s">%s</a><br>
<a href="http://127.0.0.1:6969/action/like?content=%s">like this tweet</a><br>
<a href="http://127.0.0.1:6969/action/retweet?content=%s">retweet this tweet</a><br>
</div>`
...
<body>
<a href="http://127.0.0.1:6969/writetweet">Write your own CX tweet!</a> <br>
...
func tweetById(id_ string) {
	...
	if DBG_CHECK_CACHE_ERR && len(byts) == 0 {
		cache.err = ERRORCODE_BADOBJECT
		return
	}
...
func accountById(id_ string, getTweets bool) {
	...
	if getTweets {
		...
		for i := 0; i < len(cache.account.tweets); i++ {
			...
			cache.tweet = Tweet{}
			if DBG_CHECK_CACHE_ERR && cache.err != ERRORCODE_OK {
				return
			}
		}
	}
...
func nameToAddress(t []byte) []byte {
	tc := []byte{TWEETCODE_ISN2A}
	tc = append(tc, t...)
...
func addressToName(t []byte) []byte {
	tc := []byte{TWEETCODE_ISA2N}
	tc = append(tc, t...)
...
func actionTweet(t string) {
	var b []byte
	regx := regexp.MustCompile("[0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_]")
	...
	for i := 0; i < len(t); i++ {
		if t[i] == '@' {
			...
			addr_ := nameToAddress(tstring)
			/* perform API call to get true wallet (or get nil) */
...
func buildTweet(ctweet Tweet) string {
	if DBG_CHECK_CACHE_ERR && cache.err == ERRORCODE_BADOBJECT {
		return "<p>Error: this tweet could not be displayed.</p><br>"
	}
	twttxt := ctweet.txt
	...
	for i := 0; i < len(ctweet.tags); i++ {
		...
		twttxt = strings.Replace(twttxt, "@"+str_,
			"<a href=\"http://127.0.0.1:6969/account?id="+str_+"\">@"+str_+"</a>", -1)
	...
	for i := 0; i < len(ctweet.hashs); i++ {
		...
		twttxt = strings.Replace(twttxt, "#"+str_,
			"<a href=\"http://127.0.0.1:6969/hashtag?id="+str_+"\">#"+str_+"</a>", -1)
		alreadytagged = append(alreadytagged, str_)
	}
...
func HashtagById(w http.ResponseWriter, r *http.Request) {
	...
	if DBG_CHECK_CACHE_ERR && cache.err == ERRORCODE_BADOBJECT {
		w.Write([]byte("<p>Error: this hashtag could not be displayed.</p><br>"))
		return
	}
...
func ActionTweet(w http.ResponseWriter, r *http.Request) {
	...
	switch cache.err {
	case ERRORCODE_OK:
		resp = "<p>Tweet sent!<br><a href=\"http://127.0.0.1:6969/account?id=" + cache.myaccount + "\">back to my account</p>"
	case ERRORCODE_TWEETINVALID:
		resp = "<p>Tweet is invalid!<br><a href=\"http://127.0.0.1:6969/writetweet\">back to write tweet</p>"
	case ERRORCODE_TWEETTOOLONG:
		resp = "<p>Tweet is too long!<br><a href=\"http://127.0.0.1:6969/writetweet\">back to write tweet</p>"
	}
...
func ActionLike(w http.ResponseWriter, r *http.Request) {
	...
	switch cache.err {
	case ERRORCODE_OK:
		resp = "<p>Tweet liked!<br><a href=\"http://127.0.0.1:6969/account?id=" + cache.myaccount + "\">back to my account</p>"
	case ERRORCODE_TWEETINVALID:
		resp = "<p>Tweet like is invalid!<br><a href=\"http://127.0.0.1:6969/account?id=" + cache.myaccount + "\">back to my account</p>"
	case ERRORCODE_TWEETTOOLONG:
		resp = "<p>Tweet like is too long!<br><a href=\"http://127.0.0.1:6969/account?id=" + cache.myaccount + "\">back to my account</p>"
	}
...
func ActionRetweet(w http.ResponseWriter, r *http.Request) {
	...
	switch cache.err {
	case ERRORCODE_OK:
		resp = "<p>Tweet retweeted!<br><a href=\"http://127.0.0.1:6969/account?id=" + cache.myaccount + "\">back to my account</p>"
	case ERRORCODE_TWEETINVALID:
		resp = "<p>Tweet retweet is invalid!<br><a href=\"http://127.0.0.1:6969/account?id=" + cache.myaccount + "\">back to my account</p>"
	case ERRORCODE_TWEETTOOLONG:
		resp = "<p>Tweet retweet is too long!<br><a href=\"http://127.0.0.1:6969/account?id=" + cache.myaccount + "\">back to my account</p>"
	}
...
func WriteTweet(w http.ResponseWriter, r *http.Request) {
	str := `
	<form method="POST" action="http://127.0.0.1:6969/action/tweet">
...
func GetAllTweets(w http.ResponseWriter, r *http.Request) {
	...
	if DBG_CHECK_CACHE_ERR && len(byts) == 0 {
		cache.err = ERRORCODE_BADOBJECT
		w.Write([]byte(fmt.Sprintf(docform, "<p>Tweet retweet is invalid!<br><a href=\"http://127.0.0.1:6969/account?id="+cache.myaccount+"\">back to my account</p>")))
	}
...
func handleRequests() {
	...
	http.HandleFunc("/action/tweet", ActionTweet)
	http.HandleFunc("/action/like", ActionLike)
	http.HandleFunc("/action/retweet", ActionRetweet)
	http.HandleFunc("/writetweet", WriteTweet)
	http.HandleFunc("/all", GetAllTweets)
