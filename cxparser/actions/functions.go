...
func CheckTypes(expr *ast.CXExpression) {
	...
	if expr.Operator != nil && expr.Operator.IsBuiltin && expr.Operator.OpCode == constants.OP_IDENTITY {
		for i := range expr.Inputs {
			...
			if ast.GetAssignmentElement(expr.Outputs[i]).StructType != nil {
				expectedType = ast.GetAssignmentElement(expr.Outputs[i]).StructType.Name
			} else {
				// then it's native type
				expectedType = ast.GetAssignmentElement(expr.Outputs[i]).Type.Name()
			}

			if ast.GetAssignmentElement(expr.Inputs[i]).StructType != nil {
				receivedType = ast.GetAssignmentElement(expr.Inputs[i]).StructType.Name
			} else {
				// then it's native type
