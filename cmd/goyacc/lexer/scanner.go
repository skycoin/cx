...
to ignore the returned ok value and drive a stage2 only by the arune/token id value. This is presumably the easier way for e.g. goyacc.
...
func (s *Scanner) Scan() (arune rune, ok bool) {
	for {
		...
		// moves > 1, don't confuse an easy stage2 by returning the last arune of an invalid token
		arune = unicode.ReplacementChar
		break
