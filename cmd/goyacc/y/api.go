...
// Package y converts .y (yacc[2]) source files to data suitable for a stage2
// generator.
//
// Changelog
//
...
// Action describes one cell of the stage2 table, ie. the action to be taken when
...
// from Examples"[1]. They extend the capability of a LALR stage2 to produce
...
//	`Missing package name or newline after "package"`
//
//	// A calculator stage2 might have error examples like
...
//
// It's an error if the example token sequence is accepted by the stage2, ie.
...
// lookahead is actually considered by the stage2.
//
...
// Parser describes the resulting stage2. The intended client is a stage2
...
stack:
	...
	return yystate, fmt.Errorf("stage2 stall in state %d", yystate)
}
...
// State represents one state of the stage2.
type State struct {
...
// reachable.
//
// To construct an example of a string for which the stage2 enters state s:
//
//	syms, la := s.Syms0()
//	if la != nil {
//		syms = append(syms, la)
...
// LiteralString field
//
// Some stage2 generators accept an optional literal string token associated
...
//	passed to yyerror from the stage2 will reference the literal string
...
// XError describes the stage2 state for an error by example. See [1].
type XError struct {
