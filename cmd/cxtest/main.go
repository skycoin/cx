...
func runTestCases(t *runner.TestRunner) {
	...
	t.RunEx("issue-221.cx", runner.CxSuccess, "Can't call method from package", runner.TestNone, 0)
	...
	t.RunEx("issue-270.cx", runner.CxSuccess, "Short variable declarations are not working with calls to methods or functions", runner.TestNone, 0)
	...
	t.RunEx("issue-289.cx", runner.CxCompilationError, "Panic when using a function declared in another package without importing the package", runner.TestNone, 0)
	...
	t.RunEx("issue-292.cx", runner.CxCompilationError, "Panic when calling a function from another package where the package name alias a local variable name", runner.TestNone, 0)
